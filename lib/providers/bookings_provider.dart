...
class BookingsState {
  ...
  const BookingsState({this.bookings = const [], this.isLoading = false, this.error});
  ...
  BookingsState copyWith({List<BookingItem>? bookings, bool? isLoading, String? error, bool clearError = false}) =>
      BookingsState(bookings: bookings ?? this.bookings, isLoading: isLoading ?? this.isLoading,
...
class BookingsNotifier extends StateNotifier<BookingsState> {
  ...
  Future<BookingItem> createBooking(CreateBookingRequest request) async {
    final booking = await _service.create(request);
  ...
  Future<void> cancelBooking(String id, {String? reason}) async {
    ...
    final updated = state.bookings.map((b) => b.id == id ? b.copyWith(status: 'cancelled') : b).toList();
